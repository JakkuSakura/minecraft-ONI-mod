package conservecraft.common.debug
...
object OniDebugHttpServer {
    ...
    fun ensureStarted(serverRef: MinecraftServer) {
        ...
        try {
            val bind = System.getProperty("conservecraft.http.bind", "127.0.0.1").trim()
            val port = System.getProperty("conservecraft.http.port", "38080").trim().toInt()
            ...
            executor = Executors.newFixedThreadPool(2) { runnable ->
                Thread(runnable, "conservecraft-http").apply { isDaemon = true }
            }
    ...
    private fun isEnabled(): Boolean {
        val value = System.getProperty("conservecraft.http.enabled", "true")
            .trim()
